...
test('the v2 circuit label changes the insecure configuration ID', () => {
  ...
  const params = BFV_PARAMS.insecure512
  ...
  assert.equal(configId(CIRCUIT_VERSION_LABEL), '0x19921c8c12f93c3013be57d0859f4ddcdb4464ac856a0c62be1ad617fbbd2e7d')
...
test('source hash covers compile-bearing Noir inputs but ignores active selectors', () => {
  ...
  const thresholdSecureSource = join(root, 'circuits', 'bin', 'threshold', 'rlk_generation', 'src', 'main.nr')
  const aggregationSecureSource = join(root, 'circuits', 'bin', 'recursive_aggregation', 'rlk_aggregation', 'src', 'main.nr')
  ...
  try {
    writeFiles([
      ...
      join(root, 'circuits', 'bin', 'threshold', 'rlk_generation', 'Nargo.toml'),
      thresholdSecureSource,
      join(root, 'circuits', 'bin', 'recursive_aggregation', 'rlk_aggregation', 'Nargo.toml'),
