...
use super::models::{E3Auction, IndexedBid, RoundResults, RoundStatus, TokenHolder};
...
impl<S: DataStore> RoundIndexRepository<S> {
    ...
    pub async fn record_round(&mut self, e3_id: &str) -> Result<()> {
        ...
        self.store
            .modify("_auction:round_index", |index: Option<RoundIndex>| {
    ...
    pub async fn round_ids(&self) -> Result<Vec<String>> {
        Ok(self
            .store
            .get::<RoundIndex>("_auction:round_index")
            .await
...
pub struct AuctionE3Repository<S: DataStore> {
    store: SharedStore<S>,
    e3_id: String,
}

impl<S: DataStore> AuctionE3Repository<S> {
    ...
    fn key(&self) -> String {
        format!("_auction:e3:{}", self.e3_id)
    }

    pub async fn set(&mut self, value: &E3Auction) -> Result<()> {
        let key = self.key();
        self.store
            .insert(&key, value)
            .await
    ...
    pub async fn try_get(&self) -> Result<Option<E3Auction>> {
        let key = self.key();
        self.store
            .get::<E3Auction>(&key)
            .await
    ...
    pub async fn get(&self) -> Result<E3Auction> {
        self.try_get()
            .await?
    ...
    pub async fn update<F>(&mut self, mut f: F) -> Result<E3Auction>
    where
        F: FnMut(&mut E3Auction) + Send,
    {
        let key = self.key();
        self.store
            .modify(&key, |current: Option<E3Auction>| {
    ...
    pub async fn set_balance_root(&mut self, root: String, hashes: Vec<String>) -> Result<()> {
        self.update(|r| {
            r.balance_root = Some(root.clone());
            r.token_holder_hashes = hashes.clone();
        })
        .await?;
        Ok(())
    }

    /// Records an accepted bid (idempotent on the on-chain index).
    pub async fn insert_bid(&mut self, bid: IndexedBid, ciphertext: Option<Vec<u8>>) -> Result<()> {
        self.update(|r| {
            if r.bids.iter().any(|b| b.index == bid.index) {
                return;
            }
            if let Some(ct) = &ciphertext {
                r.ciphertexts.push((bid.index, ct.clone()));
            }
            r.bids.push(bid.clone());
            r.bids.sort_by_key(|b| b.index);
        })
        .await?;
        Ok(())
    }
    ...
    pub fn snapshot_of(round: &E3Auction) -> &[TokenHolder] {
        &round.snapshot
    }
