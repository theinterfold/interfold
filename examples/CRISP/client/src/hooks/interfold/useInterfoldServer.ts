...
const InterfoldEndpoints = {
  ...
  GetArchivePage: `${INTERFOLD_API}/state/archive`,
...
export const useInterfoldServer = () => {
  ...
  const endpoints = useMemo(() => {
    const getCurrentRound = () =>
      fetchData<CurrentRound, { requesters: string[] }>(
        GetCurrentRound,
        'post',
        { requesters: ROUND_REQUESTERS },
    ...
    const getArchivePage = (cursor?: string) =>
      fetchData<ArchivePage, { requesters: string[]; cursor?: string; limit: number }>(InterfoldEndpoints.GetArchivePage, 'post', {
        requesters: ROUND_REQUESTERS,
        cursor,
        limit: 12,
      })
    const getWebResultByRound = (round_id: string) =>
      fetchData<PollRequestResult, { round_id: string }>(GetWebResult, 'post', { round_id }, { suppressNotFound: true })
