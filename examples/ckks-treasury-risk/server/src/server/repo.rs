...
use super::models::{E3Treasury, IndexedSubmission, RoundResults, RoundStatus};
...
impl<S: DataStore> RoundIndexRepository<S> {
    ...
    pub async fn record_round(&mut self, e3_id: &str) -> Result<()> {
        ...
        self.store
            .modify("_treasury:round_index", |index: Option<RoundIndex>| {
    ...
    pub async fn round_ids(&self) -> Result<Vec<String>> {
        Ok(self
            .store
            .get::<RoundIndex>("_treasury:round_index")
            .await
...
pub struct TreasuryE3Repository<S: DataStore> {
    store: SharedStore<S>,
    e3_id: String,
}

impl<S: DataStore> TreasuryE3Repository<S> {
    ...
    fn key(&self) -> String {
        format!("_treasury:e3:{}", self.e3_id)
    }

    pub async fn set(&mut self, value: &E3Treasury) -> Result<()> {
        let key = self.key();
        self.store
            .insert(&key, value)
            .await
    ...
    pub async fn try_get(&self) -> Result<Option<E3Treasury>> {
        let key = self.key();
        self.store
            .get::<E3Treasury>(&key)
            .await
    ...
    pub async fn get(&self) -> Result<E3Treasury> {
        self.try_get()
            .await?
    ...
    pub async fn update<F>(&mut self, mut f: F) -> Result<E3Treasury>
    where
        F: FnMut(&mut E3Treasury) + Send,
    {
        let key = self.key();
        self.store
            .modify(&key, |current: Option<E3Treasury>| {
    ...
    /// Records an accepted submission (idempotent on the on-chain index).
    pub async fn insert_submission(
        &mut self,
        sub: IndexedSubmission,
        ciphertexts: Option<(Vec<u8>, Vec<u8>, Vec<u8>)>,
    ) -> Result<()> {
        self.update(|r| {
            if r.submissions.iter().any(|b| b.index == sub.index) {
                return;
            }
            if let Some((f, rv, m)) = &ciphertexts {
                r.ciphertexts
                    .push((sub.index, f.clone(), rv.clone(), m.clone()));
            }
            r.submissions.push(sub.clone());
            r.submissions.sort_by_key(|b| b.index);
        })
        .await?;
        Ok(())
    }
