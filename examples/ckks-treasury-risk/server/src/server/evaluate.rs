...
//! window closes (or every registered DAO submitted), run the treasury policy over the indexed
//! ciphertext triples (read back from calldata) under the committee's level-0 relin key and
//! publish the evaluated ciphertext on-chain, which triggers the threshold decryption.
//! Refuses to evaluate with fewer than [`ckks_treasury_program::MIN_DAOS`] submissions.

use crate::config::CONFIG;
use crate::server::contract::ciphertext_commitment;
use crate::server::models::RoundStatus;
use crate::server::repo::TreasuryE3Repository;
use alloy::primitives::{Bytes, U256};
use ckks_treasury_program::{DaoInputs, MIN_DAOS};
...
pub async fn evaluate_and_publish(store: SharedStore<impl DataStore>, e3_id: &str) -> Result<()> {
    ...
    if round.ciphertexts.len() != round.submissions.len() {
        return Err(eyre!(
            "{} submission(s) indexed but only {} ciphertext triple(s) recovered from calldata",
            round.submissions.len(),
            round.ciphertexts.len()
        ));
    }
    if round.ciphertexts.len() < MIN_DAOS {
        return Err(eyre!(
            "{} submission(s); at least {MIN_DAOS} DAOs must submit before the aggregate risk is evaluated",
            round.ciphertexts.len()
        ));
    }
    let inputs: Vec<DaoInputs> = round
        .ciphertexts
        .iter()
        .map(|(_, f, r, m)| (f.clone(), r.clone(), m.clone()))
        .collect();
    let rlk_dir = relin_key_dir(&CONFIG.relin_key_dir, CONFIG.chain_id, e3_id);
    for f in ckks_treasury_program::ceremony_key_files() {
    ...
    info!(
        "[e3_id={e3_id}] evaluating {} DAOs: F = Σ forward(x_i), R = Σ reversed(w∘x_i) — ONE ct×ct under rlk_level_0, one rescale, + Σ mask(m_i), opens at level {}",
        inputs.len(),
        ckks_treasury_program::opening_level()
    );
    let t0 = Instant::now();
    let output =
        tokio::task::spawn_blocking(move || ckks_treasury_program::evaluate(&inputs, &rlk_dir))
            .await
