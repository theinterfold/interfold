...
#
# One-command dev boot for the CKKS private-matching app (CRISP `dev.sh` shape):
#   anvil → deploy contracts (mocks + CkksMatchingE3Program + ParamSet 5) → 5 ciphernodes →
#   coordination server (:8093) → Vite client (:5176).
#
#   pnpm dev:up                      # from examples/ckks-matching
#
# Ctrl-C tears everything down. Set CKKS_MATCHING_NO_CLIENT=1 to skip the client (e2e drives it).
...
export CLIENT_PORT="${CLIENT_PORT:-5176}"
...
for c in user_data_encryption_ckks_ct0_ps5 user_data_encryption_ckks_ct1_ps5 ckks_matching_validity_ps5; do
...
if [[ "${CKKS_MATCHING_NO_CLIENT:-0}" == "1" ]]; then
