...
//! open, deadline hook) → `SubmissionPublished` (submission accepted by the five-leg gate;
//! both ciphertexts recovered from calldata; once BOTH parties are in, evaluate right away) →
//! deadline: evaluate + publish → `PlaintextOutputPublished` (decode the 64 coefficients;
//! `score = −opened[0]`).

use crate::config::CONFIG;
use crate::server::contract::{ciphertexts_from_calldata, MatchingProgram, SubmissionPublished};
use crate::server::evaluate::evaluate_and_publish;
use crate::server::models::{IndexedSubmission, RoundResults, RoundStatus};
use crate::server::repo::{now_secs, MatchingE3Repository};
use alloy::primitives::B256;
use alloy::sol_types::SolEvent;
use ckks_matching_program::{Role, PARTIES};
...
async fn register_e3_requested(
    ...
    indexer
        .add_event_handler(move |event: E3Requested, ctx| {
            ...
            let store = ctx.store();
            async move {
                let repo = MatchingE3Repository::new(store, &e3_id);
...
async fn register_committee_published(
    ...
    indexer
        .add_event_handler(move |event: CommitteePublished, ctx| {
            ...
            let store = ctx.store();
            async move {
                let mut repo = MatchingE3Repository::new(store.clone(), &e3_id);
...
async fn handle_deadline(e3_id: String, store: SharedStore<impl DataStore>) -> eyre::Result<()> {
    ...
    if round.submissions.len() < PARTIES {
        warn!(
            "[e3_id={e3_id}] window closed with {} submission(s) (< {PARTIES}) — not evaluating",
            round.submissions.len()
        );
        repo.set_error(format!(
            "round closed with {} submission(s); both parties (A and B) must submit",
            round.submissions.len()
        ))
        .await?;
        return Ok(());
...
/// `SubmissionPublished` carries no bytes — only the two keccaks — so this is a RAW log
    ...
    indexer
        .add_raw_log_handler(move |log, ctx| {
            ...
            async move {
                ...
                let program = MatchingProgram::new(&CONFIG.http_rpc_url, &CONFIG.private_key, &CONFIG.e3_program_address).await?;
                let ciphertexts = match program.transaction_input(tx_hash).await {
                    Ok(input) => match ciphertexts_from_calldata(&input, ev.vectorCiphertextHash, ev.maskCiphertextHash) {
                        Ok(ct) => Some(ct),
                        Err(e) => {
                ...
                // The slot is assigned ON-CHAIN (position in the registered pair) and IS the role.
                let index: u64 = ev.index.to::<u64>();
                let role = Role::from_index(index).map_err(|e| eyre::eyre!("{e:#}"))?;
                info!(
                    "[e3_id={e3_id}] SubmissionPublished slot {index} ({role:?}, {} layout): party {} tx {tx_hash} cts {} + {} bytes (5 Honk proofs verified on-chain)",
                    role.layout(),
                    ev.party,
                ...
                repo.insert_submission(
                    IndexedSubmission {
                        index,
                        role,
                        publisher: ev.party.to_string(),
                        transaction_hash: tx_hash.to_string(),
                        block,
                        vector_ciphertext_hash: ev.vectorCiphertextHash.to_string(),
                        mask_ciphertext_hash: ev.maskCiphertextHash.to_string(),
                        m_commitment_vec: ev.mCommitmentVec.to_string(),
                ...
                .await?;
                // Both parties in: nothing else can arrive (one submission per registered
                // sender, two senders), so evaluate now instead of waiting for the window.
                let round = repo.get().await?;
                if round.status == RoundStatus::Active && round.submissions.len() == PARTIES && round.ciphertexts.len() == PARTIES {
                    info!("[e3_id={e3_id}] both parties submitted — evaluating now");
                    repo.set_status(RoundStatus::Evaluating).await?;
    ...
    indexer
        .add_event_handler(move |event: PlaintextOutputPublished, ctx| {
            ...
            async move {
                ...
                let opened = ckks_matching_program::decode_opened(&event.plaintextOutput)
                    .map_err(|e| eyre::eyre!("{e:#}"))?;
                let score = ckks_matching_program::score_from_opened(&opened)
                    .map_err(|e| eyre::eyre!("{e:#}"))?;
                info!(
                    "[e3_id={e3_id}] PlaintextOutputPublished: {} coefficients opened — score = −c_0 = {score:.4} (coefficients 1.. are mask-hidden cross terms)",
                    opened.len()
                );
...
pub async fn start_indexer(
    ...
    info!(
        "ckks-matching: creating indexer (interfold {interfold_address}, program {program_address})"
    );
    ...
    info!("ckks-matching: indexer listening");
