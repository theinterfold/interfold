...
//! - `POST /rounds/request`                  open a round (admin; body = `{partyA, partyB}`)
//! - `GET  /rounds/{id}`                     round detail (parties, submissions, results, timings)
//! - `GET  /rounds/{id}/public-key`          committee CKKS pk (hex) once published
//! - `GET  /rounds/{id}/slot/{address}`      the party's registered slot + role (A/B) + layout
//! - `POST /rounds/{id}/evaluate`            force evaluation now (admin / dev; needs both parties)
//! - `GET  /rounds/{id}/result`              the opened score once finished
//!
//! Nothing here accepts a submission: they go straight from the party's wallet to the
//! program contract. The server never sees a vector or a mask.
...
async fn status() -> impl Responder {
    ...
    HttpResponse::Ok().json(json!({
        ...
        "paramSet": ckks_matching_program::PARAM_SET,
        "k": ckks_matching_program::K,
        "parties": PARTIES,
        "maskWidth": ckks_matching_program::MASK_WIDTH,
        "maskBound": ckks_matching_program::MASK_BOUND,
        "entryBound": ckks_matching_program::ENTRY_BOUND,
        "openingLevel": ckks_matching_program::opening_level(),
        "ceremonyKeys": ckks_matching_program::ceremony_key_files(),
        "outputCount": ckks_matching_program::OUTPUT_COUNT,
        "outputDecimals": ckks_matching_program::OUTPUT_DECIMALS,
    }))
}

fn summary(r: &crate::server::models::E3Matching) -> RoundSummary {
    RoundSummary {
        e3_id: r.e3_id.clone(),
        status: r.status,
        input_window: r.input_window,
        party_a: r.parties.first().cloned().unwrap_or_default(),
        party_b: r.parties.get(1).cloned().unwrap_or_default(),
...
async fn request_round(data: web::Data<AppData>, body: web::Json<RoundRequest>) -> impl Responder {
    ...
    match open_round(data.store(), req.party_a, req.party_b, req.duration_secs).await {
...
async fn round_detail(data: web::Data<AppData>, path: web::Path<String>) -> impl Responder {
    ...
    HttpResponse::Ok().json(RoundDetail {
        ...
        param_set: ckks_matching_program::PARAM_SET,
        k: ckks_matching_program::K,
...
async fn slot(data: web::Data<AppData>, path: web::Path<(String, String)>) -> impl Responder {
    ...
    {
        Some(index) => {
            let role = match Role::from_index(index as u64) {
                Ok(r) => r,
                Err(e) => return internal(e),
            };
            HttpResponse::Ok().json(SlotResponse {
                address: round.parties[index].clone(),
                index: index as u32,
                role,
                layout: role.layout(),
            })
        ...
        None => HttpResponse::NotFound()
            .json(json!({ "error": format!("{address} is not a registered party of round {id}") })),
    }
}
...
async fn evaluate_now(data: web::Data<AppData>, path: web::Path<String>) -> impl Responder {
    ...
    if round.submissions.len() < PARTIES {
        return bad(format!(
            "{} submission(s); both parties (A = slot 0, B = slot 1) must submit before the score is evaluated",
