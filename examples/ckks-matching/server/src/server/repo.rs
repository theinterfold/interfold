...
use super::models::{E3Matching, IndexedSubmission, RoundResults, RoundStatus};
...
impl<S: DataStore> RoundIndexRepository<S> {
    ...
    pub async fn record_round(&mut self, e3_id: &str) -> Result<()> {
        ...
        self.store
            .modify("_matching:round_index", |index: Option<RoundIndex>| {
    ...
    pub async fn round_ids(&self) -> Result<Vec<String>> {
        Ok(self
            .store
            .get::<RoundIndex>("_matching:round_index")
            .await
...
pub struct MatchingE3Repository<S: DataStore> {
    store: SharedStore<S>,
    e3_id: String,
}

impl<S: DataStore> MatchingE3Repository<S> {
    ...
    fn key(&self) -> String {
        format!("_matching:e3:{}", self.e3_id)
    }

    pub async fn set(&mut self, value: &E3Matching) -> Result<()> {
        let key = self.key();
        self.store
            .insert(&key, value)
            .await
    ...
    pub async fn try_get(&self) -> Result<Option<E3Matching>> {
        let key = self.key();
        self.store
            .get::<E3Matching>(&key)
            .await
    ...
    pub async fn get(&self) -> Result<E3Matching> {
        self.try_get()
            .await?
    ...
    pub async fn update<F>(&mut self, mut f: F) -> Result<E3Matching>
    where
        F: FnMut(&mut E3Matching) + Send,
    {
        let key = self.key();
        self.store
            .modify(&key, |current: Option<E3Matching>| {
    ...
    pub async fn insert_submission(
        ...
        self.update(|r| {
            ...
            if let Some((v, m)) = &ciphertexts {
                r.ciphertexts.push((sub.index, v.clone(), m.clone()));
            }
