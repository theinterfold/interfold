...
#[derive(Deserialize)]
pub struct Config {
    /// Round-opener key: requests E3s, registers the two parties, publishes the evaluated
    /// ciphertext. It NEVER sees a vector or a mask — submissions are sent by each party from
    /// its own wallet and the plaintexts never leave the browser.
    pub private_key: String,
    pub http_rpc_url: String,
    pub ws_rpc_url: String,
    pub chain_id: u64,
    pub interfold_address: String,
    /// `CkksMatchingE3Program` — the five-leg gate every submission must pass.
