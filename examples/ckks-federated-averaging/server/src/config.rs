...
#[derive(Deserialize)]
pub struct Config {
    /// Round-opener key: requests E3s, registers rounds, publishes the evaluated ciphertext.
    /// It NEVER sees an update or a sample count — updates are encrypted and proven in the
    /// client's browser and sent from the client's own wallet.
    pub private_key: String,
    pub http_rpc_url: String,
    pub ws_rpc_url: String,
    pub chain_id: u64,
    pub interfold_address: String,
    /// `CkksFedAvgE3Program` — the five-leg gate every update must pass.
    ...
    /// Seconds the update window stays open once the committee key is published.
