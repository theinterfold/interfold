...
#
# One-command dev boot for the CKKS federated-averaging app (CRISP `dev.sh` shape):
#   anvil → deploy contracts (mocks + CkksFedAvgE3Program + ParamSet 5) → 5 ciphernodes →
#   coordination server (:8095) → Vite client (:5178).
#
#   pnpm dev:up                      # from examples/ckks-federated-averaging
#
# Ctrl-C tears everything down. Set CKKS_FEDAVG_NO_CLIENT=1 to skip the client (e2e drives it).
...
export CLIENT_PORT="${CLIENT_PORT:-5178}"
...
for c in user_data_encryption_ckks_ct0_ps5 user_data_encryption_ckks_ct1_ps5 ckks_fedavg_validity_ps5; do
...
if [[ "${CKKS_FEDAVG_NO_CLIENT:-0}" == "1" ]]; then
