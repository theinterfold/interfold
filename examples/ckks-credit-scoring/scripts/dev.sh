...
#
# One-command dev boot for the CKKS credit-scoring app (CRISP `dev.sh` shape):
#   anvil → deploy contracts (mocks + CkksCreditE3Program + ParamSet 4) → 5 ciphernodes →
#   coordination server (:8092) → Vite client (:5175).
#
#   pnpm dev:up                      # from examples/ckks-credit-scoring
#
# Ctrl-C tears everything down. Set CKKS_CREDIT_NO_CLIENT=1 to skip the client (e2e drives it).
# Requires: the release node + tool binaries (`cargo build --release --bin interfold` at the
# root), compiled circuits (`circuits/bin/threshold/target/*_ps4.json`), the ps4 committee
...
export CLIENT_PORT="${CLIENT_PORT:-5175}"
...
for c in user_data_encryption_ckks_ct0_ps4 user_data_encryption_ckks_ct1_ps4 ckks_credit_validity_ps4; do
...
if [[ "${CKKS_CREDIT_NO_CLIENT:-0}" == "1" ]]; then
