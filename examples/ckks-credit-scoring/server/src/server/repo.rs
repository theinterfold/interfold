...
use super::models::{E3Credit, IndexedApplication, RoundResults, RoundStatus};
...
impl<S: DataStore> RoundIndexRepository<S> {
    ...
    pub async fn record_round(&mut self, e3_id: &str) -> Result<()> {
        ...
        self.store
            .modify("_credit:round_index", |index: Option<RoundIndex>| {
    ...
    pub async fn round_ids(&self) -> Result<Vec<String>> {
        Ok(self
            .store
            .get::<RoundIndex>("_credit:round_index")
            .await
...
pub struct CreditE3Repository<S: DataStore> {
    store: SharedStore<S>,
    e3_id: String,
}

impl<S: DataStore> CreditE3Repository<S> {
    ...
    fn key(&self) -> String {
        format!("_credit:e3:{}", self.e3_id)
    }

    pub async fn set(&mut self, value: &E3Credit) -> Result<()> {
        let key = self.key();
        self.store
            .insert(&key, value)
            .await
    ...
    pub async fn try_get(&self) -> Result<Option<E3Credit>> {
        let key = self.key();
        self.store
            .get::<E3Credit>(&key)
            .await
    ...
    pub async fn get(&self) -> Result<E3Credit> {
        self.try_get()
            .await?
    ...
    pub async fn update<F>(&mut self, mut f: F) -> Result<E3Credit>
    where
        F: FnMut(&mut E3Credit) + Send,
    {
        let key = self.key();
        self.store
            .modify(&key, |current: Option<E3Credit>| {
    ...
    pub async fn set_issuer_root(&mut self, root: String, hashes: Vec<String>) -> Result<()> {
        self.update(|r| {
            r.issuer_root = Some(root.clone());
            r.leaf_hashes = hashes.clone();
        })
        .await?;
        Ok(())
    }

    /// Records an accepted application (idempotent on the on-chain index).
    pub async fn insert_application(
        ...
        self.update(|r| {
            if r.applications.iter().any(|b| b.index == app.index) {
                return;
            }
            if let Some((z, m)) = &ciphertexts {
                r.ciphertexts.push((app.index, z.clone(), m.clone()));
            }
            r.applications.push(app.clone());
            r.applications.sort_by_key(|b| b.index);
        })
        .await?;
        Ok(())
    }
