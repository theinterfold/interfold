...
#[derive(Deserialize)]
pub struct Config {
    /// Round-opener key: requests E3s, sets issuer roots, publishes the evaluated ciphertext.
    /// It NEVER sees a feature or a mask — applications are sent by applicants from their own
    /// wallets and the masks never leave the browser.
    pub private_key: String,
    pub http_rpc_url: String,
    pub ws_rpc_url: String,
    pub chain_id: u64,
    pub interfold_address: String,
    /// `CkksCreditE3Program` — the five-leg gate every application must pass.
    pub e3_program_address: String,
    pub ciphernode_registry_address: String,
    pub fee_token_address: String,
    /// Where the ciphernodes write the joint relin keys
    /// (`<dir>/<chain_id>:<e3_id>/rlk_level_{1,2}.bin`). Set the same
    /// `CKKS_RELIN_KEY_DIR` for the nodes, or point this at one node's
    /// `<data dir>/<node>/ckks/relin-keys`.
    pub relin_key_dir: String,
    /// Seconds the application window stays open once the committee key is published.
