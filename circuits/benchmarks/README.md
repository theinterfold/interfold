...
**All `(preset, committee)` pairs are supported.** The parity matrices and smudging constants in
`circuits/lib/src/configs/committee/<name>/` are generated by the `generate_parity_matrices` Rust
binary. `scripts/build-circuits.ts` invokes the binary whenever the committee changes. The values
are deterministic functions of the committee and BFV preset, so each pair compiles with the same
parameters that Rust uses for witness generation.

**Skipping the build step:** `pnpm check:committee` runs in the pre-push hook and fails the push if
`active.nr`, `utils.ts`, `.active-preset.json`, or any generated committee artifact disagrees with
what `build:circuits --committee <active>` would produce. Always use
`pnpm build:circuits --committee` to switch; manual edits to those files will trip the check.
