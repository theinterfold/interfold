...
- Mainnet CRISP activation is one paused and drained governance batch. It upgrades Interfold to the
  ...
  CRISP, binds CRISP, and raises the required node protocol version. The partial CRISP-only builder
  must not run on mainnet. Old nodes become ineligible in the activation transaction. The CRISP
  program and ciphertext verifier image IDs must both equal the RISC Zero image generated by the
