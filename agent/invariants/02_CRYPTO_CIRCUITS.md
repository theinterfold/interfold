...
- A circuit release archive that supports current deployments must include every
  `insecure/{minimum,micro,small}`, `secure-8192/{minimum,micro,small}`, and `secure-16384/minimum`
  pair. Each pair has a build stamp with the exact preset, committee, and source hash. A pair hash
  includes shared compile-bearing sources and only the circuit packages enabled for that preset. A
  secure-16384-only circuit change must not invalidate an `insecure` or `secure-8192` pair.
  `checksums.json` and `SHA256SUMS` must cover the archive contents. Nodes select the artifact
  directory from the E3's on-chain parameter set and committee size. **Gap:** `download_circuits`
  accepts an archive without `checksums.json` (`crates/zk-prover/src/backend/download.rs`).
- The secure-16384 l-BFV CRS and URS use constants version 1 and different fixed SHA-256 domain
  labels. The runtime and circuit generator expand both values with
  `CommonRandomPolyVec::from_seed`. The CRS, URS, Garner rows, parameter set, level, and row
  serialization form one compatibility unit. A change to one value requires a new constants version
  and regenerated circuit artifacts. — `crates/fhe-params/src/lbfv.rs`;
  `generate_config_modules.rs`; `flow-trace/04`
...
- WASM witness JSON must encode integers outside JavaScript's safe range as decimal strings before
  `JSON.parse`. Rounding a BFV coefficient changes the encryption witness and invalidates P3.
  SDK proof bundles use the witness degree and a minimum-committee build. The active local preset
  does not select a bundled artifact. — `crates/bfv-client/src/client.rs`; `flow-trace/04`
...
- **Client PK commitment binding (C-01):** Serialized PK event bytes are an untrusted transport
  ...
  Secure-16384 consumers decode the version-3 l-BFV key envelope, reconstruct its complete public
  key from the RLK, and validate both keys against the fixed CRS and URS. Consumers store the key
  only when the recomputed legacy or envelope commitment equals the on-chain proof value.
  Proof-backed committee publication rejects unbound or commitment-mismatched key bytes. Public-key
  candidates are bounded and gated to request-time committee members while the E3 remains in
  `KeyPublished`. Retained expelled members can still repair transport, but their bytes receive no
  extra trust. Terminal E3s cannot create new durable assemblies after cleanup. Consumers accept at
  most one candidate per member, so an invalid candidate cannot block a valid candidate from another
  member. — INDEX concerns #33, Z-31
