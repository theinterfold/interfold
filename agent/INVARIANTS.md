...
- Mainnet CRISP activation is one paused and drained governance batch. It upgrades Interfold to the
  ...
  both equal the RISC Zero image generated by the same release source for that historical activation.
  OpenVM requires a separate paused, drained migration with an identity derived from the new guest,
  VM configuration, and Halo2 verifier; the RISC Zero activation scripts must not activate it.
  Requests remain paused until
...
- **Every E3 program must compare the proof's input root against its own root.**
  `OpenVmBfvCiphertextVerifier` and the legacy `Risc0BfvCiphertextVerifier` take no `inputRoot`
  argument and constrain none. A program that
