...
> computation looks like inside an OpenVM guest, and how the final proof is verified on-chain.
>
...
inputs. OpenVM proves CRISP's Secure Process, which validates the input commitments, applies the
slot and parent policy, and aggregates the selected ciphertexts. The addition primitive is:
...
The OpenVM worker produces an application proof, recursively aggregates it, and generates the
Halo2 EVM proof. Configure the guest, keys, and worker as described in
[Compute Provider](/compute-provider). The full guest proves the input policy and domain-bound
journal, not only the addition primitive above. CRISP deployment requires a real compute verifier.
...
program. CRISP's implementation checks the OpenVM proof:
...
function verify(
  ...
  // Rebuild the OpenVM journal from the expected round context.
...
The journal contains nine consecutive 32-byte ABI words: 288 bytes in total.
`OpenVmComputeProof.journal` builds that layout. The guest reveals its SHA-256 digest, and the
receipt adapter binds that digest to the configured application and VM commitments.
...
sequenceDiagram
    ...
    CP->>CP: Secure Process<br/>input policy, homomorphic sum, OpenVM proof
