...
it because the deployment record lists `MockE3Program`, although that program is not enabled. See
[Verifiers in use on mainnet](#verifiers-in-use-on-mainnet).
...
### Verifiers in use on mainnet

The `Interfold` contract stores one verifier of each kind for each encryption scheme. The table
shows the values for the BFV scheme (`keccak256("fhe.rs:BFV")`) at block 26,057,073 (25 September
2026). Each E3 keeps the verifiers that were set when it was requested.

| Verifier   | Contract                      | Address                                      | Set at block | Read with                         |
| ---------- | ----------------------------- | -------------------------------------------- | ------------ | --------------------------------- |
| Ciphertext | `Risc0BfvCiphertextVerifier`  | `0x80D217d3b2e16fF2ecc178cC75655C773895c549` | 26004622     | `getCiphertextVerifier(schemeId)` |
| Decryption | `BfvDecryptionVerifierRouter` | `0xA66CAb7AE230698b2a9ee1E0E967BaD4651FeEa7` | Not recorded | `getDecryptionVerifier(schemeId)` |
| Public key | `BfvPkVerifierRouter`         | `0x7CD10057c25674bd5666A9deEE3193f1b29563d1` | Not recorded | `getPkVerifier(schemeId)`         |

- The ciphertext verifier checks a RISC Zero Groth16 proof for the guest image ID
  `0x9d3e21fd7cc08e629cb467e5ca6bfc8e1a645b483bfd91b6b1648ced259110fc`. Read it with `imageId()`.
- Each router has three routes (`routeCount()`, `routeAt(index)`). All routes point to
  `BfvDecryptionVerifier` or `BfvPkVerifier` contracts.
- The deployment record `mainnet-protocol.deployment.json` does not include these values. It lists
  the verifiers of the first deployment.

<Callout type='info'>
  The first mainnet deployment set `DeployableMockCiphertextVerifier` as the ciphertext verifier at
  block 25,788,974 (19 August 2026). That contract accepts every proof. The contract owner replaced
  it with a `Risc0BfvCiphertextVerifier` at block 25,813,839 (22 August 2026), and again at block
  26,004,622 (18 September 2026). The first mainnet E3 was requested at block 26,012,772, so no
  mainnet E3 used the mock verifier. The first E3 program, `MockE3Program`, is registered but not
  enabled (`e3Programs` returns `false`). `CRISPProgram` is enabled.
</Callout>

The manifest still marks mainnet with `mocks: true`, because the deployment record lists
`MockE3Program`. For this reason, `interfold config check` prints the mock-deployment note for
mainnet, although the verifiers in use are not mocks.
...
<Callout type='warning'>
  ...
  Sepolia deployment is upgraded, use a local network to run ciphernodes.
</Callout>
...
| DeployableMockCiphertextVerifier | `contracts/test/DeployableMockCiphertextVerifier.sol` | A test verifier that accepts every ciphertext proof. It was the mainnet ciphertext verifier from 19 to 22 August 2026.                                                                                                                                 |
