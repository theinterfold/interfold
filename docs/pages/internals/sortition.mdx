...
operators, and so on. One owner's backups cannot occupy all the first retry positions. Backups
still submit during the normal window; this is not a new staged submission protocol. If owner
history is incomplete, the runtime logs a warning and allows all eligible operators to submit.
The on-chain cap still applies. Neither the buffer nor the fallback guarantees enough online owners.
...
submits if shortlisted and it has local capacity. The contract enforces the owner cap; the aggregator
...
The existing `BondOwnerSet` events supply ownership history in a separate, versioned snapshot.
On upgrade, startup reconstructs missing chain projections from the global event log through its
snapshot cursor. Existing node and recovery schemas remain unchanged. A restored ticket intent
keeps its original ticket number and finalization rank; startup does not rank that request again.
...
not a contract-wide admission guard, an online check, or a capacity reservation. Direct callers
must perform their own preflight, and state can change before a request is mined.
