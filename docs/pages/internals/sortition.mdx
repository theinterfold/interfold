...
The sortition selects more candidate nodes than the finalized committee needs. In the Rust runtime,
the legacy field `threshold_m` is the polynomial threshold `T`, so decryption requires
`threshold_m + 1` shares. The field `threshold_n` is the finalized committee size `N`. The local
candidate-set size is:
...
Where `safety_factor` depends on the `T/N` ratio:
