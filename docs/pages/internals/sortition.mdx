...
The runtime computes every eligible operator's best ticket, then ranks owners by their best
operator. The existing `N + buffer` limit counts distinct owners, not operators. Every operator
within those shortlisted groups can submit if it has local capacity. This retains backups when an
owner's best operator is offline, without sending tickets from the rest of the owner groups.

Finalization-attempt ranks visit each shortlisted owner's best operator first, then their second
operators, and so on. One owner's backups cannot occupy all the first retry positions. Backups still
submit during the normal window; this is not a new staged submission protocol. If owner history is
incomplete, the runtime logs a warning and allows all eligible operators to submit. The on-chain cap
still applies. Neither the buffer nor the fallback guarantees enough online owners.

Owner checkpoints use the source block's timestamp, not the time the node received the event. Older
local owner records without that timestamp are not used to filter candidates. The fallback can
increase transaction count and provisional capacity use. A missing update in an otherwise populated
cache can still give the node a stale shortlist: chain validation protects the cap, not committee
availability. Operators must complete chain synchronization before serving requests. Restart
preserves a generated ticket and its finalization rank, including when aggregation is disabled.
...
Each node computes its best ticket and checks whether its request-time owner is shortlisted. It
submits if shortlisted and it has local capacity. The contract enforces the owner cap; the
aggregator does not approve tickets.

```rust
// Pseudocode for SortitionBackend::get_submission_index
let eligible = nodes.filter(|n| n.is_active && n.available_tickets > 0);
let limit = n + calculate_buffer_size(t, n);
let candidates = shortlist_owners_with_backups(eligible, request_owners, limit, e3id, seed);
...
Newly decoded `BondOwnerSet` logs become `BondOwnerSetAt` records that retain the original block
timestamp. These records supply the separate v2 owner snapshot. Startup reconstructs missing chain
projections from the global event log through its snapshot cursor. It does not import old records
that contain only ingestion time. Existing node and recovery payloads remain unchanged. A restored
ticket intent keeps its original ticket number and finalization rank; startup does not rank that
request again.

`committee:new` checks distinct eligible owners at a single chain block before fee approval and
payment. It stops if fewer than `N` owners qualify or the check fails. This is a request-tool check,
not a contract-wide admission guard, an online check, or a capacity reservation. Direct callers must
perform their own preflight, and state can change before a request is mined.
...
| **Formation candidates**    | N + buffer owner groups with their backup operators; N distinct owners needed  |
