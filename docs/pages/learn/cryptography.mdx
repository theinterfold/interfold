...
Three values define the committee shape. **N** is the finalized on-chain committee size, **H** is
the number of mutually ready dealer contributions that form the DKG key, and decryption requires
**T+1** valid shares. The active aggregator proposes one canonical, ascending H-member roster **S**
after the early proofs are verified. When DKG finishes, the network publishes a **threshold public
key** built from exactly those H contributions.

The supported `(N, T, H)` configurations are Minimum `(3, 1, 2)`, Micro `(9, 4, 5)`, and Small
`(19, 9, 14)`. Small can omit five dealers before roster selection and can lose four of its 14
selected decryption participants later while still reaching `T+1=10` shares.
...
circuits that handle keys and decryptions. When the job is done, at least **T+1** members recorded
by the accepted DKG publish **partial decryptions**, and an **aggregator** combines them into
plaintext **without** ever reassembling the full secret key in one place.
...
- **P1 — Distributed key generation** is the heavy chapter. Each ciphernode builds its **secret key
  ...
  generation, share computation, encryption, and the local decrypt-and-aggregate step. After at
  least H mutually ready dealer contributions exist, the active aggregator proposes the fixed roster
  `S` that all later DKG proofs use.

- **P2 — Public key aggregation** is deliberately short: the active aggregator sums the H selected
  public key shares into the single **threshold public key**. Circuit
...
- **P4 — Threshold decryption** closes the loop after homomorphic evaluation. Ciphernodes anchored
  by the accepted DKG produce partial decryptions; circuits
...
sequenceDiagram
    ...
    loop Each participating node before roster selection
        CMT->>CMT: Individual sk/pk + proof (C0)
        CMT->>CMT: Contribution + smudging + proofs (C1, C2)
        CMT->>CMT: Encrypt and exchange shares + proofs (C3)
    end
    CMT-->>AGG: Signed readiness reports
    AGG-->>CMT: Accepted H-dealer roster
    loop Each accepted roster member
...
enter the FHE engine before P4. **Robustness** means the DKG can select H mutually ready dealers
without waiting for all N members. Invalid proofs remain attributable, but a local omission timeout
alone is not evidence that permits slashing.
...
1. **DKG recursive proof**: the aggregator recursively verifies exactly H accepted-roster C0–C4 fold
   chains together with **C5**, which aggregates the same H public-key contributions. It also binds
   the proof to the full N-member committee hash. The proof is verified on-chain by
   ...
   at `publishCommittee`.
2. **Decryption recursive proof**: the aggregator recursively verifies **C6** proofs from exactly
...
Individual nodes do not post their inner proofs on-chain. Each selected node folds its own C0–C4
chain into a `NodeDkgFold` proof, **signs** the surfaced commitments, and relays the signed fold
output to the aggregator via P2P gossip. The aggregator then assembles the recursive DKG proof from
the H roster members' fold outputs and submits it together with a **fold-attestation bundle**.
...
4. **Roster cardinality**: the bundle must contain exactly one attestation per selected party in the
   H-row proof, with `partyId` strictly ascending (no duplicates, no partial subsets).

This closes the per-operator attribution gap: the aggregator cannot present a valid DKG proof
without also producing one signed attestation per selected party, each bound to the operator
...
1. **Voter quorum and signatures**: ≥ `H` committee-member voters (sorted ascending, accused
...
[**C3**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/share_encryption)
(`dkg/share_encryption`) then encrypts each share under the recipient’s individual key. The witness
has N recipient slots, so the maximum proof count scales like **N × (N − 1)** per track and modulus.
When a recipient's C0 key is missing at the cutoff, the sender fills that proof slot with a
non-delivered placeholder encrypted under its own C0 key. At a Fiat–Shamir challenge point, the BFV
encryption algebra it checks matches the usual two-leg ciphertext (per limb), schematically:
...
Finally,
[**C4**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/share_decryption)
(`dkg/share_decryption`) runs after the network accepts one H-member roster: it proves the opened
values from that roster match **C2**'s commitments, aggregates per modulus, and forwards commitments
toward threshold decryption
([**C6**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/share_decryption)).
...
Here, `\mathcal{H}` is the accepted H-member roster, not every member of the original N-member
committee. Nodes publish signed readiness lists after C2/C3 verification. The active aggregator
chooses an ascending H-member subset that every selected member reports as ready, and peers validate
that roster before C4. The existing aggregator failover moves this coordination role to the next
eligible party if needed. The roster is fixed when C4 starts; this release recovers restarted roster
members but does not replace a member that disappears permanently after acceptance.

### P2 — Public key aggregation

[**C5**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/pk_aggregation)
(`threshold/pk_aggregation`) is the bridge from H committed contributions to one threshold public
key: it re-checks each selected contribution against **C1**, then aggregates. The **first** leg is
summed across the accepted roster; the **second** leg is the same CRS polynomial **a** for every
party, so **pk1_agg** is **not** a sum—it must equal each party’s **pk1** (see `verify_pk_for_basis`
vs `verify_pk1` in `pk_aggregation.nr`):
