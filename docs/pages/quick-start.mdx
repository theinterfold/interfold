...
Configure `program.openvm` with the repository, worker executable, and deployment-local prover
configuration. Prepare a guest and keys for your program policy as described in
[Compute Provider](/compute-provider), then build the native service:

```bash
interfold program compile
```

This command does not generate guest keys. Contract deployment also needs the application
commitments and the checksummed Halo2 verifier artifact or a checked existing verifier address.
The CRISP guest is specific to CRISP; a template policy needs its own guest and deployment.
