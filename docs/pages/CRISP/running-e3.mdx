...
For an Avail-backed round, the CLI reads `CRISPProgram.earliestVotingStart()` and schedules a fixed
voting start after the worst-case VRF, ticket, and DKG windows. It adds
`VOTING_START_BUFFER_SECONDS` for transaction mining. Publishing the committee key early does not
open the vote early. `E3_DURATION` begins at the scheduled start and includes the voting period plus
the Avail finalization tail.
...
The program service logs the OpenVM worker's application, aggregation, and EVM proof stages. A
successful worker exit is followed by callback delivery. Confirm the ciphertext publication
transaction and the E3's `CiphertextReady` stage; a worker timing line alone does not establish
on-chain acceptance.
...
3. **Computation**: After input finalization, OpenVM proves the Secure Process over the encrypted inputs
