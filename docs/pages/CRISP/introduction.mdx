...
`CRISPVerifier.sol` for a census posted as a Merkle root, and `CRISPOnchainVerifier.sol` for one
read from token balances on chain. Both declare a contract named `HonkVerifier`, so they are
distinguished by file. Votes are stored in the Merkle tree with slot addresses mapping to vote
