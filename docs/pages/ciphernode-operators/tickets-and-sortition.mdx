...
- Admitted under the cooldown and pause policy at the request snapshot
- No exit in progress
- At least one unreserved local workload slot

Governance can configure an admission delay for new registrations and bond-owner changes. A transfer
proposal does not start the delay; acceptance does. Re-registration starts a new delay. These times
are recorded even when governance disables the cooldown.

An admission pause is different from disabling the cooldown. A pause keeps waiting, new, and
owner-changed positions out while already eligible positions can continue. The waiting time
continues during a pause. Disabling the cooldown while admissions are open removes the waiting
requirement, but does not bypass the owner seat cap, collateral, or slash restrictions. Existing E3s
keep their request-time policy.

After a policy change, call the permissionless `refreshOperatorStatus(operator)` or batch
`refreshOperatorStatuses(operators)` before requesting an E3. Refresh a waiting position after its
delay expires so it counts toward committee capacity. Expiry does not send a transaction
automatically. `isActive` reports collateral/release readiness; `eligibilityAt` includes admission.
An unchanged pre-upgrade position keeps admission because its original registration time is not
available.

Changes to collateral, ticket, or required-release rules require a check of every existing
registration before another E3 request. Anyone can run the batch refresh, including for an offline
operator. An inactive result still completes that operator's check. Checking the same operator again
or registering a new operator cannot skip an existing one. After the last check, wait for a later
block timestamp and check `committeeOwnerCapacity(latest.timestamp - 1)`.

Before governance pauses admissions after an eligibility configuration change:

1. Refresh every registered operator, including operators expected to remain inactive.
2. Wait for a later block timestamp.
3. Check that `committeeOwnerCapacity(latest.timestamp - 1)` covers the required committee size.
4. Pause admissions.
5. Refresh the frozen eligible pool, then wait for a later timestamp before requesting an E3.

An operator that was inactive at the pause cutoff cannot enter through a later refresh. If the
pre-pause refresh was missed, governance must unpause, refresh, and pause again in the order above.
