...
All eligible nodes with local capacity can submit. The runtime no longer excludes candidates with an
`N + buffer` cutoff. That cutoff could contain one owner's operators and exclude owners needed to
fill the committee. This increases ticket transaction costs; a submission does not guarantee a seat
or a reward.
...
members. Check owner capacity before requesting an E3. The request-time active-node count does not
measure distinct owners; an underfilled round fails at finalization.
...
- `"This node was NOT selected for sortition"` - Your node lacks eligibility or local capacity
...
- `"Ranking eligible ticket submissions; the contract selects distinct bond owners"` - Candidate
  ranking started
