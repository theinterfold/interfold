...
- **Committee configuration**: Choose a supported committee enum. Each enum fixes total members `N`,
  the mutually ready DKG roster `H`, and decryption threshold `T`. Do not supply arbitrary threshold
  values.
- **Timing windows**: Set `inputWindow[0]` after the worst-case VRF, ticket, and DKG setup time. Set
  `inputWindow[1]` late enough for inputs and data-availability finalization. Include a transaction
  buffer.
...
- **Push logic to validators**: Implement `IE3Program.publishInput` so ZK proofs and policy checks
...
- **Proof hooks**: When using OpenVM, keep witness blobs small and
