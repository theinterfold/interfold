...
| ----------------------- | ------------------- | ------ | -------------- | -------- | -------------------------- |
| `INSECURE_THRESHOLD`    | `0`                 | 128    | 2              | ❌ None  | Fast local or testnet work |
...
request an E3 with `paramSet: 1` but initialize the SDK with `INSECURE_THRESHOLD`, the ciphertext
will be encrypted under the wrong parameters and decryption will fail.
...
const ct = await encryptNumber(42n, pk, 'INSECURE_THRESHOLD')
```
...
`--preset insecure --committee minimum` (see `packages/interfold-sdk/scripts/compile-circuits.sh`),
so they only match `INSECURE_THRESHOLD` and a minimum-size committee.
