...
//! Accelerated implementation of the exact BN254 Poseidon2 permutation used by SAFE.
...
#[cfg(feature = "openvm")]
use openvm_algebra_guest::{moduli_macros::moduli_declare, IntMod};
...
#[cfg(feature = "openvm")]
impl Copy for Bn254Scalar {}

#[cfg(feature = "openvm")]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct FastField(Bn254Scalar);
#[cfg(not(feature = "openvm"))]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct FastField([u32; 8]);

impl FastField {
    #[cfg(any(not(feature = "openvm"), test))]
    const ZERO: Self = Self::new([0; 8]);

    #[cfg(feature = "openvm")]
    const MONTGOMERY_R: Self = Self::new([
        Fr::R.0[0] as u32,
        (Fr::R.0[0] >> 32) as u32,
        Fr::R.0[1] as u32,
        (Fr::R.0[1] >> 32) as u32,
    ...
    #[cfg(feature = "openvm")]
    const MONTGOMERY_R_INVERSE: Self = Self::new([
        0x6db1194e, 0xdc5ba005, 0xe111ec87, 0x090ef5a9, 0xaeb85d5d, 0xc8260de4, 0x82c5551c,
        0x15ebf951,
    ]);

    const fn new(words: [u32; 8]) -> Self {
        #[cfg(not(feature = "openvm"))]
        {
            Self(words)
        }
        #[cfg(feature = "openvm")]
        {
            ...
            while index < 8 {
                ...
                let mut byte = 0;
                while byte < 4 {
                    bytes[index * 4 + byte] = word[byte];
                    byte += 1;
                }
                index += 1;
            }
            Self(Bn254Scalar::from_const_bytes(bytes))
        }
    }
    ...
    fn words(self) -> [u32; 8] {
        ...
        {
            let mut words = [0; 8];
            for (word, bytes) in words.iter_mut().zip(self.0.as_le_bytes().chunks_exact(4)) {
    ...
    fn from_ark(value: Fr) -> Self {
        ...
        #[cfg(feature = "openvm")]
        {
            value.multiply(Self::MONTGOMERY_R_INVERSE)
        }
    }
    ...
    fn into_ark(self) -> Fr {
        ...
        #[cfg(feature = "openvm")]
        let words = {
            let montgomery = self.multiply(Self::MONTGOMERY_R);
            assert!(
                montgomery.0.is_reduced(),
                "The field value is not canonical"
            );
            montgomery.words()
        };
        let bigint = BigInt([
        ...
        #[cfg(feature = "openvm")]
        {
            // The product is reduced above and already uses the Montgomery representation.
            Fr::new_unchecked(bigint)
        }
    }
...
#[cfg(feature = "openvm")]
#[inline(always)]
fn pow_five_assign(value: &mut FastField) {
    let base = value.0;
    value.0.square_assign();
    value.0.square_assign();
    value.0 *= &base;
}
...
#[inline(always)]
fn matmul_external(state: &mut [FastField; WIDTH]) {
    let mut t0 = &state[0].0 + &state[1].0;
    let mut t1 = &state[2].0 + &state[3].0;
    let mut t2 = state[1].0.double();
    t2 += &t1;
    let mut t3 = state[3].0.double();
    t3 += &t0;
    t1.double_assign();
    t1.double_assign();
    t1 += &t3;
...
fn internal_round(state: &mut [FastField; WIDTH], constant: FastField) {
    ...
    pow_five_assign(&mut state[0]);
    let mut sum = &state[0].0 + &state[1].0;
    sum += &state[2].0;
    sum += &state[3].0;
    for (value, diagonal) in state.iter_mut().zip(&MAT_DIAG_M_1) {
        value.0 *= &diagonal.0;
        value.0 += &sum;
    }
}
...
mod tests {
    ...
    #[test]
    fn accelerated_field_conversion_matches_canonical_words() {
        let mut rng = ChaCha8Rng::seed_from_u64(0xc0eff);
        let mut inputs = vec![Fr::from(0), Fr::from(1), -Fr::from(1)];
        inputs.extend((0..100).map(|_| random_field(&mut rng)));
        for input in inputs {
            let accelerated = FastField::from_ark(input);
            assert_eq!(accelerated.into_ark(), input);
            let words = accelerated.words();
            ...
            assert_eq!(canonical, input.into_bigint());
        }
    }
