...
impl ThresholdPlaintextAggregation {
    ...
    pub(crate) fn verify_ckks_shares_match_c6_commitments(
        ...
        // Widths at the CIPHERTEXT'S level (the opening-level rule): the
        // C6 proof was generated by the circuit whose configs are the
        // moduli that remain there.
        let Ok(bounds) = C6CkksBounds::compute_at_level(&preset, ct.level) else {
            return all_mismatched("could not compute C6-CKKS bounds");
        };
        let Ok(bits) = C6CkksBits::compute_at_level(&preset, &bounds, ct.level) else {
