...
/// One real party share at the set's OPENING level on ParamSet `set`,
/// its ciphertext, and the `d_commitment` the C6-CKKS witness builder
/// would emit (hashed at that level's `d_native_bit`).
#[allow(clippy::type_complexity)]
fn fixture(set: u8) -> Result<(Vec<u8>, Vec<u8>, Vec<u8>, [u8; 32])> {
    let params = ckks_params_for_on_chain_param_set(set)?;
    let level = ckks_opening_level_for_param_set(set)?;
    ...
    let mut ct = pk.try_encrypt(&encoder.encode(&[42.5, -17.25], 0)?, &mut rng)?;
    ...
    let bounds = C6Bounds::compute_at_level(&preset, level)?;
    let bits = C6Bits::compute_at_level(&preset, &bounds, level)?;
