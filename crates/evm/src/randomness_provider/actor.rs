...
/// Parse a randomness log and retry once with a new provider after a read failure.
async fn parse_fulfillment_with_reconnect<P: Provider + Clone + 'static>(
    ...
    warn!(
        ...
        error = %first_error,
        "Randomness verification failed; reconnecting the read provider"
    );
    let replacement = match factory().await {
        ...
        Ok(replacement) => {
            warn!(
                ...
                "Refusing a randomness provider for another chain"
            );
            return (provider, first);
        }
        Err(reconnect_error) => {
...
mod tests {
    ...
    async fn dead_transport_is_replaced_before_rejecting_a_fulfillment() {
        ...
        let event = parsed
            .expect("the new provider must verify the fulfillment")
    ...
    async fn reconnect_refuses_a_provider_for_another_chain() {
        ...
        let error = parsed.expect_err("a provider for another chain must not be used");
        assert!(error
            .to_string()
