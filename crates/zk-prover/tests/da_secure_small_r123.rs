...
//
//! r123 - the `DecryptionAggregator` (DA) PRODUCTION-KECCAK-DOMAIN anchor.
//!
//! r122 RAN-PROVED the DA at secure-8192/small with a (domain_hi, domain_lo)
//! = (1, 2) STAND-IN and flagged the production keccak domain as the one
//! intentionally-simplified input. This round closes that line: the domain
//! is now derived via e3_committee_hash::decryption_domain_limbs exactly as
//! production does (crates/multithread/src/multithread.rs:452-457):
//!   domain_hash = keccak256(abi.encode(chainId, interfold_address, e3_id,
//!                committee_hash, ciphertext_output_hash, committee_pk))
//!   (domain_hi, domain_lo) = (hi_128_bits, lo_128_bits)
//! and every C6 inner + the c6_fold chain + the DA EVM witness carry that
//! REAL derived domain. The DA cross-assert (fold-published domain == DA
//! public domain) runs on genuine keccak limbs end to end. All other inputs
//! are IDENTICAL to r122 (same coherent-world shape, same stage tree,
//! zero compile).
//!
...
//! [r123: the REAL production keccak-derived domain, see header], and (c) C6 inner i's
...
//!
//! DOMAIN (r123: REAL keccak, no stand-in): (domain_hi, domain_lo) for all
//! 10 inners = the production keccak-derived limbs (see header), computed
//! from the SAME 19 committee addresses the DA consumes as topNodes, the
//! SAME aggregated pk world encrypts with (pk commitment), and the SAME
//! ciphertext bytes (ciphertext output hash). The production keccak-derived domain binds the proof to (chainId,
...
//! OTHER cross-assert runs on real RAN data. r123 keeps EVERY other input
//! identical to r122 and swaps ONLY the stand-in domain for the real
//! production keccak-derived domain, then re-runs the FULL prove (C6x10 +
//! C7 + c6_fold + DA EVM + EVM verify).
//!
//! STAGE TREE (reused from r122 byte-conserved tree - zero compile this round):
//!     $E3_R123_STAGE_ROOT/secure-8192/small/
...
//!         cargo test --release -p e3-zk-prover --test da_secure_small_r123 -- --nocapture
...
use e3_committee_hash::{
    decryption_domain_limbs, DecryptionDomainContext,
};
...
/// deployment values; the DERIVATION path is the production one, see header).
...
fn stage_root() -> PathBuf {
    match std::env::var("E3_R123_STAGE_ROOT") {
        Ok(p) if !p.is_empty() => PathBuf::from(p),
        _ => panic!(
            "E3_R123_STAGE_ROOT unset - run poc/r122/stage_da_secure_small_r122.py (r122 stage tree, reused) first"
        ),
    }
}
...
async fn da_secure_small_proves_and_verifies() {
    ...
    assert!(
        preset_tree.is_dir(),
        "stage tree {} missing - run poc/r122/stage_da_secure_small_r122.py (r122 stage tree, reused) first",
        preset_tree.display()
    );
    ...
    let _ = prover.cleanup("e3-r123-da");

    // ---- verify the DA EVM proof (on-chain verifier path) ----
    let ti = Instant::now();
    let ok = prover
        .verify_proof_with_variant(
            &da_proofs[0], "e3-r123-da", 1, CircuitVariant::Evm, &ad,
        )
    ...
    println!(
        "\nDecryptionAggregator PRODUCTION-FIELD ANCHOR GREEN (REAL keccak DOMAIN) @secure-8192/small \
         (N=19/T=9/H=10/L=3).\n  Build shared world {:.2}s; leg compute {:.2}s @4c \
         (7.8 GiB / 30-60m budget).\n  r123: the r122 stand-in domain (1,2) is REPLACED by the production \
         keccak-derived domain end to end (C6 x10 + C7 + c6_fold + DA Evm + \
         EVM verify, all on the SAME derived domain).",
        build, total
    );
}