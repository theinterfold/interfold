...
//
//! r65 — I5a PRODUCTION WIRING seam test (r35 VK-pattern, M7x c3b arm, secure-8192/small).
//!
//! What it verifies (the load-bearing legs of the r64-UNBLOCKED production wiring):
//!
//!   (1) BOTH arms of the c3ab seam produce their final proofs on the REAL production
//!       geometry of node P=1 (W_1 = {0..57}\{3,4,5}, anchor W_1[0]=0; C3_SLOTS=57):
//!         c3b arm = `generate_c3_merge_m7x`  -> circuit C3FoldBatchMergeM7x (the M7x
//!                 8-top-level-prove merge; 175 c3_fold-EXACT public fields)
//!         c3a arm = `generate_sequential_c3_fold` over a same-size 39-slot fan-out
//!                 -> circuit C3Fold (the unchanged legacy arm)
//!   (2) THE C3AB SEAM (the r35 claim, un-verified until now): build the c3ab witness
//!       EXACTLY as the production `C3abFoldWitness` does (`node_dkg_fold.rs`) and pin
//!       c3b against the M7x VK (.vk/.vk_hash of `C3FoldBatchMergeM7x`) while c3a stays
//!       on the c3_fold VK — i.e. c3ab_fold's in-circuit `verify_honk_proof_non_zk`
//!       accepts a DIFFERENT-SIZE merge proof's VK as a runtime witness input. This is
//!       the "VK-rebuild-only drop-in" step: no c3ab recompile (c3ab json is UNTOUCHED,
//!       mtime-preserved) and no node_fold/aggregator change.
//!       ABI fact making this safe (source RAN): `UltraHonkVerificationKey = [Field; 115]`
//!       (bb_proof_verification/lib.nr, FIXED 115 fields, circuit-size-independent) —
//!       every fold VK on disk is exactly 115 fields (3680 B), M7x included.
//!   (3) `verify_fold_proof(c3ab)` bb offline verify PASS — closes the seam with an
//!       independent verifier, not just witness gen.
//!   (4) Corruption check: the c3ab c3a/c3b column tails equal the arm proofs' tails
//!       (all 57 rows) and the two pinned key-hash publics equal the c3_fold / M7x VK
//!       key hashes.
//!
//! STAGING (already in place from r61/r63): secure-8192/small artifacts for the leaf +
//! kernel + c3_fold + b10 + b3 + M7x (all C3_SLOTS=57) + c3ab json (175/arm). c3ab
//! additionally needs its `.vk_recursive` (poc/r65_c3ab_vk.sh) — the test fail-fasts if
//! the staged c3ab json is not the small build or the C3_SLOTS guards mismatch (r63
//! stale-artifact class). Run:
//!   cargo test --release -p e3-zk-prover --test m7x_wiring_seam_tests_r65 -- --nocapture
//!   (quiet box, release profile, UNCONTESTED; ~84 secure inners dominate ~20-30 min.)
...
/// Node P=1 production C3b chain: W_1 = {0..57}\{3,4,5} ascending (54 slots).
const NODE_P: u32 = 1;
/// C3a fan-out for the seam: a 30-slot contiguous block {3..33} (N=19, L=3 — a real
/// (recipient, modulus) fan-out shape; the c3a arm stays the SEQUENTIAL fold, so no
/// M7x-eligibility claim rides on c3a's shape, only that c3ab consumes BOTH arms).
...
#[tokio::test]
async fn r65_c3ab_wiring_seam_m7x_c3b_c3fold_c3a() {
    ...
    let w_b = w_p(NODE_P);
    assert_eq!(w_b.len(), 54, "W_1 must be 54 slots");
    ...
    println!(
        "  c3a arm sequential (1 kernel + 38 c3_fold steps) wall = {c3a_wall:.1}s  fields = 175  circuit = c3_fold  RAN"
    );
    ...
    let c3ab_ok = prover
        .verify_fold_proof(&c3ab, "e3-r65-c3ab", 1, &ad)
    ...
    println!(
        "  r65 seam TOTAL (inners {inners_wall:.1}s + c3b {m7x_wall:.1}s + c3a {c3a_wall:.1}s + c3ab {c3ab_wall:.1}s) = {:.1}s  RAN",
